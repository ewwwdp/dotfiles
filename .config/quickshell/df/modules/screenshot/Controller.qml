...
Scope {
    ...
    Process {
        ...
        onExited: code => {
            if (code == 0) {
                ...
                console.info("screenshot failed " + path);
                cleanupProc.running = true;
            }
        }
    }
    ...
    Process {
        id: magickProc
        command: ["magick", root.path, "-crop", `${selection.normal.width}x${selection.normal.height}+${selection.normal.x}+${selection.normal.y}`, "-quality", "70", "-page", "0x0+0+0" // removes page size and shot position
            , root.path,]
